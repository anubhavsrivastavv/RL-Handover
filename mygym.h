...
public:
  ...
  //static void PerformHandOver(Ptr<MyGymEnv> entity, uint32_t currentCellId,other parameters );
  void CollectChannelOccupation(uint32_t chanId, uint32_t occupied);//not necessary
  bool CheckIfReady();
  void ClearObs();

private:
  
  /* variables used for HO */
  Time m_interval = Seconds(0.1);
  uint32_t m_currentCellID;//check the no. of bits assigned for CellID in LTE
  uint32_t m_noOfBuckets;//no. of buckets for RSRP values to make it discrete
  std::vector<uint32_t> m_cellRsrpOccupationState;//State to be sent to RL agent
  uint32_t m_rsrpValue=0;
